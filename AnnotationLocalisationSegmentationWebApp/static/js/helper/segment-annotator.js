...
function (Layer, segmentation, morph) {
  ...
  // Previous Exportthe annotation in data URL.
    /*
...
//Pixel Filtration Algorithm
Annotator.prototype.export = function () {
 var image_id = xy;
 var annotator_username = annotator_name;
 var instauser = insta_user;


  //var ctx = this.layers.annotation.canvas.getContext('2d');
  ...
  // Code  to fetch pixel label pairs.
      ...
      var arrayoflabels = [
        ...
        for(var y =0; y<this.currentHistoryRecord+1;y++){
          var record = this.history[y];
          for(var x =0; x<record.pixels.length;x++){
            var lab = record.next[x];
           // alert(lab);
            var pix = record.pixels[x];
            //alert(pix);
            arrayoflabels[lab].push(pix);
          }
        }

      //alert(arrayoflabels);

      // Code to fetch lablecode + RGB
      var colorsRGB = [];
      var color;
      var labelCodes = [],
      data = this.layers.annotation.imageData.data;
      for (var i = 0; i < data.length; i += 4) { //for all annotation data get labels
        var label = _getEncodedLabel(data, i);

        if (labelCodes.indexOf(label) < 0) {
          labelCodes.push(label); // Push all the labels

          for(var s = 0; s < labelCodes.length;s++)
          {
                color = this.colormap[_getEncodedLabel(labelCodes,s)];
                if(color != null)
                {
      ...
      this.layers.annotation.applyColormap(this.colormap).render();

      //Fetch canvas and then convert toDataURL
     ...
     var segmentedDataPixels = {Blouse_Tunics: arrayoflabels[3], Coats: arrayoflabels[4],Dresses:arrayoflabels[5],Jeans: arrayoflabels[6],Jackets:arrayoflabels[7],
      ...
      //var allRGB = colorsRGB //which we are not sending
        ...
        var string = JSON.stringify(segmentedDataPixels);

       // var csrftoken = $('meta[name=csrf-token]').attr('content');

     var data1 = {segmenteddata:segmentedDataPixels,Labels:labelCodes,Total:labelCount,RGB:colorsRGB,ImageID:image_id, Annotatorname:annotator_username,Instaname:instauser,
         image:saveimage};
